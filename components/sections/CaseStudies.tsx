...
function SideCard({ c, index }: { c: typeof OTHERS[0]; index: number }) {
  ...
  return (
    <motion.div
      ...
      className="group relative bg-white px-6 py-5 transition-colors duration-300 hover:bg-[#FBFAFF]"
      style={{
        borderRadius: 18,
        border: "1px solid rgba(17,17,26,0.07)",
      }}
    >
      {/* Cabeçalho: segmento + nome, com o ponto de cor como assinatura discreta */}
      <div className="flex items-start justify-between gap-3 mb-4">
        <div className="min-w-0">
          <p
            className="font-body font-semibold text-[10px] uppercase tracking-[0.22em] mb-1"
            style={{ color: c.accentColor }}
          >
            {c.segment}
          </p>
          <h3 className="font-headline font-bold text-brand-text text-lg leading-tight truncate">
            {c.name}
          </h3>
      ...
      {/* Número protagonista (Fraunces itálica, peso fino) + sua legenda */}
      <div className="flex items-end gap-3 mb-3">
        ...
        <span className="font-body text-gray-500 text-xs leading-snug pb-1.5 max-w-[150px]">
          {hero.label}
        </span>
      </div>

      {/* Demais resultados como linha tipográfica separada por divisor (sem caixinhas) */}
      <div
        className="flex flex-wrap items-baseline gap-x-4 gap-y-1 pt-3"
        style={{ borderTop: "1px solid rgba(17,17,26,0.06)" }}
      >
        {rest.map((s, j) => (
          <span key={j} className="font-body text-[13px] text-gray-600">
            <span className="font-headline font-bold text-brand-text">{s.value}</span>{" "}
...
export default function CaseStudies() {
  return (
    <section
      ...
      style={{ background: "linear-gradient(160deg, #F5F1EA 0%, #FFFFFF 45%, #EDE9FE 100%)" }}
    >
      {/* Linha de topo degradê primário */}
      <div
        aria-hidden="true"
        className="absolute top-0 left-0 right-0 h-[2px]"
      ...
      <div
        aria-hidden="true"
        className="pointer-events-none absolute right-[2%] top-[5%] font-headline font-bold italic select-none hidden lg:block"
        style={{
          fontSize: "clamp(10rem, 20vw, 22rem)",
          lineHeight: 1,
          background: "linear-gradient(135deg, rgba(106,72,244,0.10) 0%, rgba(20,62,102,0.06) 100%)",
          WebkitBackgroundClip: "text",
          backgroundClip: "text",
          WebkitTextFillColor: "transparent",
          color: "transparent",
      ...
      <div
        ...
        style={{
          backgroundImage: "radial-gradient(circle at 1px 1px, rgba(106,72,244,0.10) 1px, transparent 0)",
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <FadeInSection className="mb-10 lg:mb-16">
          <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6">
            <div>
              ...
              <h2 className="font-headline font-bold text-brand-text text-3xl sm:text-4xl lg:text-[3.2rem] leading-[1.05] tracking-tight">
                Resultados reais de{" "}
            ...
            <p className="font-body text-gray-600 text-base leading-relaxed max-w-sm lg:text-right">
              ...
              <span className="text-brand-text font-semibold">previsibilidade nas vendas.</span>
            </p>
          </div>
        </FadeInSection>
        ...
        <FadeInSection delay={0.15}>
          ...
          >
            <p className="font-headline font-bold italic text-brand-text text-xl lg:text-2xl text-center sm:text-left">
